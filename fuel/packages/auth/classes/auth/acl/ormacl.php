			...
			foreach ( $current_roles as $role ) {
				...
				elseif ($role->filter == 'R') {
					...
					foreach ( $role->permissions as $permission ) {
						...
						$revoked_rights [$permission->area] [$permission->permission] = array_merge ( $revoked_rights [$permission->area] [$permission->permission], array_intersect_key ( $role->rolepermission ['[' . $role->id . '][' . $permission->id . ']']->permission->actions, array_flip ( $role->rolepermission ['[' . $role->id . '][' . $permission->id . ']']->actions ) ) );
					}
				} 				
				...
				// standard role, add it to the current rights set
					...
					foreach ( $role->permissions as $permission ) {
						...
						$current_rights [$permission->area] [$permission->permission] = array_merge ( $current_rights [$permission->area] [$permission->permission], array_intersect_key ( $role->rolepermission ['[' . $role->id . '][' . $permission->id . ']']->permission->actions, array_flip ( $role->rolepermission ['[' . $role->id . '][' . $permission->id . ']']->actions ) ) );
					}
				}
			}
			...
			if (is_array ( $current_rights )) {
				if ($user) {
					...
					foreach ( $user->group->permissions as $permission ) {
						isset ( $current_rights [$permission->area] [$permission->permission] ) or $current_rights [$permission->area] [$permission->permission] = array ();
						$current_rights [$permission->area] [$permission->permission] = array_merge ( $current_rights [$permission->area] [$permission->permission], array_intersect_key ( $user->group->grouppermission ['[' . $user->group_id . '][' . $permission->id . ']']->permission->actions, array_flip ( $user->group->grouppermission ['[' . $user->group_id . '][' . $permission->id . ']']->actions ) ) );
					}
					...
					if ($user) {
						foreach ( $user->permissions as $permission ) {
							isset ( $current_rights [$permission->area] [$permission->permission] ) or $current_rights [$permission->area] [$permission->permission] = array ();
							$current_rights [$permission->area] [$permission->permission] = array_merge ( $current_rights [$permission->area] [$permission->permission], array_intersect_key ( $user->userpermission ['[' . $user->id . '][' . $permission->id . ']']->permission->actions, array_flip ( $user->userpermission ['[' . $user->id . '][' . $permission->id . ']']->actions ) ) );
						}
					}
				}
			}
